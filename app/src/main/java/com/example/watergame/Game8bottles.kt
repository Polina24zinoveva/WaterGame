...
private const val countBottle = 8
...
private var colors = arrayOf("0", "0", "0", "0", "0", "0", "0")
private val colorsRainbow = arrayOf("#174ea9", "#BC2F82", "#f0ca3e", "#3fb39d", "#f24e4a", "#d07c1d", "#a2d883")
private val colorsPink = arrayOf("#af2156", "#f5a3c6", "#ec4a9b", "#db60a4", "#f8d0d0", "#822566", "#bc51ac")
private val colorsBLue = arrayOf("#001b6d", "#82b3dc", "#009fd3", "#00507d", "#5861a4", "#004a55", "#534ED9")
private val colorsGreen = arrayOf("#2d953e", "#005335", "#c5e1a4", "#01ae81", "#004123", "#0b9051", "#37b97d")
private val colorsRed = arrayOf("#e60026", "#66000b", "#dd6a52", "#b2243a", "#a73b2f", "#560122", "#ef5350")


private val bottles = mutableListOf(mutableListOf<String>(), mutableListOf<String>(), mutableListOf<String>(), mutableListOf<String>(), mutableListOf<String>(), mutableListOf<String>(), mutableListOf<String>(), mutableListOf<String>())
...
class Game8bottles : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_game8bottles)
        ...
        if (outplayClick) {
            val sharedPref8 = getSharedPreferences("my_preferences8", Context.MODE_PRIVATE)
            //распоковка нужной расстановки цветов
            firstBottleString = sharedPref8.getString("firstBottleString", "").toString()
            secondBottleString = sharedPref8.getString("secondBottleString", "").toString()
            thirdBottleString = sharedPref8.getString("thirdBottleString", "").toString()
            fourthBottleString = sharedPref8.getString("fourthBottleString", "").toString()
            fifthBottleString = sharedPref8.getString("fifthBottleString", "").toString()
            sixthBottleString = sharedPref8.getString("sixthBottleString", "").toString()
            seventhBottleString = sharedPref8.getString("seventhBottleString", "").toString()
            ...
            val sharedPref7 = getSharedPreferences("my_preferences7", Context.MODE_PRIVATE)

            firstBottleString = sharedPref7.getString("firstBottleString", "").toString()
            secondBottleString = sharedPref7.getString("secondBottleString", "").toString()
            thirdBottleString = sharedPref7.getString("thirdBottleString", "").toString()
            fourthBottleString = sharedPref7.getString("fourthBottleString", "").toString()
            fifthBottleString = sharedPref7.getString("fifthBottleString", "").toString()
            sixthBottleString = sharedPref7.getString("sixthBottleString", "").toString()
            ...
            val countColorsInBottle = mutableListOf(mutableListOf<Int>(), mutableListOf<Int>(), mutableListOf<Int>(), mutableListOf<Int>(), mutableListOf<Int>(), mutableListOf<Int>(), mutableListOf<Int>())
            for (j in 0..countBottle - countEmptyBottle - 1) { //иду по бутылкам
                ...
                if (bottles[j].size == 4 && bottles[j][0] == bottles[j][1]  &&  bottles[j][1] == bottles[j][2]  &&  bottles[j][2] == bottles[j][3]){
                    for (i in countBottle - countEmptyBottle - 1 downTo 0) { //иду по цветам
                        if (bottles[i].size == 4 && (bottles[i][0] != bottles[i][1]  ||  bottles[i][1] != bottles[i][2]  ||  bottles[i][2] != bottles[i][3] || bottles[i][0] != bottles[i][3] )){
                            var v = bottles[i][bottles[i].size - 1]
                            var v2 = bottles[j][bottles[j].size - 1]
                            bottles[i].removeAt(bottles[i].size - 1)
                            bottles[j].removeAt(bottles[j].size - 1)
                            bottles[j] += v
                            bottles[i] += v2
                            break
                        }
    ...
    private fun changeStyleAndTheme(){
        ...
        if (!colors.contentEquals(oldColors)) {
            ...
            val bottlesWithNewStyleColors = mutableListOf(mutableListOf<String>(), mutableListOf<String>(), mutableListOf<String>(), mutableListOf<String>(), mutableListOf<String>(), mutableListOf<String>(), mutableListOf<String>(), mutableListOf<String>())
